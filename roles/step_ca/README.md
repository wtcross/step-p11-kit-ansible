...
It pulls files from the `step-ca-p11-kit-images` repository using a pinned default ref and copies them to the target user's config directories:
...
- Every value in `step_ca_dns_names` is present as a DNS SAN entry in the certificate read from `step_ca_intermediate_cert_local_path`.
...
| `step_ca_images_repo_name` | GitHub repo for upstream asset source | `step-ca-p11-kit-images` |
...
| `step_gid` | Service account GID | `1001` |
...
- New instances default to container listener port `9000`.
- Existing instances keep the listener port already present in persisted `ca.json`.
- `step_ca_https_port` controls only the host-published port.

- `~/.config/p11-kit-server/<instance>.env`
- `~/.config/step-ca/<instance>.env`

When generated env file content changes, the role restarts:

- `p11-kit-server@<instance>.target`
- `step-ca-p11-kit@<instance>.service`

The role creates an instanced quadlet source symlink for each deployment:

- `~/.config/containers/systemd/step-ca-p11-kit@<instance>.container` -> `step-ca-p11-kit@.container`

The role manages an instance-level port drop-in:

- `~/.config/containers/systemd/step-ca-p11-kit@<instance>.container.d/10-ports.conf`
  ...
  roles:
    ...
    - role: wtcross.step.step_ca
      vars:
        ...
        step_ca_dns_names:
          - "ca.example.com"
        step_ca_host_private_key_pkcs11_uri: "pkcs11:token=IntermediateCA;id=%01;object=intermediate;type=private?module-path=/usr/lib64/pkcs11/opensc-pkcs11.so"
        step_ca_root_cert_local_path: "./certs/root.crt"
        step_ca_intermediate_cert_local_path: "./certs/intermediate.crt"
